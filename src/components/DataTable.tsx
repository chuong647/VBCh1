...
export const DataTable: React.FC<DataTableProps> = ({ data, theme }) => {
  ...
  if (data.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 px-4 text-center bg-white/50 backdrop-blur-sm rounded-3xl border-2 border-dashed border-slate-200 animate-in fade-in zoom-in duration-500">
        <div className="p-4 bg-blue-50 text-blue-500 rounded-full mb-4">
          <FileSearch size={48} strokeWidth={1.5} />
        </div>
        <h3 className="text-xl font-bold text-slate-800">Chưa có dữ liệu trích xuất</h3>
        <p className="text-slate-500 mt-2 max-w-xs mx-auto">Vui lòng tải lên file PDF để bắt đầu quá trình bóc tách.</p>
      </div>
    );
  }
            ...
            {data.map((doc, index) => (
              ...
              >
                ...
                <td className={`${borderClass} ${cellPadding} align-top text-center page-range-cell`}>
                  {doc.pageRange || doc.startPage || <span className="empty-field">—</span>}
                </td>
              </tr>
            ))}
          </tbody>
